...
(function (global, factory) {
  ...
  ["link", "image", "image-gallery", "video"], ["clean"] // remove formatting button
  ];
  ...
  var script = {
    ...
    methods: {
      ...
      setupQuillEditor: function setupQuillEditor() {
        ...
        this.quill = new Quill(this.$refs.quillContainer, editorConfig); // this.quill.clipboard.addMatcher(Node.TEXT_NODE, (node, delta) => {
      ...
      handleTextChange: function handleTextChange(delta, oldContents) {
        ...
        if (this.useCustomImageGalleryHandler) this.handleImageGalleryRemoved(delta, oldContents);
        if (this.useCustomCaptionHandler) this.handleCaptionRemoved(delta, oldContents);
      },
      handleImageRemoved: function handleImageRemoved(delta, oldContents) {
        var _this2 = this;

        var currentContents = this.quill.getContents();
        var deletedContents = currentContents.diff(oldContents);
      ...
      handleImageGalleryRemoved: function handleImageGalleryRemoved(delta, oldContents) {
        var _this3 = this;
        ...
        var operations = deletedContents.ops;
        operations.map(function (operation) {
          if (operation.insert && operation.insert.hasOwnProperty("image-gallery")) {
            var image = operation.insert.image;

            _this3.$emit("image-gallery-removed", image);
          }
        });
      },
      handleCaptionRemoved: function handleCaptionRemoved(delta, oldContents) {
        var _this4 = this;
        ...
        var operations = deletedContents.ops;
        operations.map(function (operation) {
          if (operation.insert && operation.insert.hasOwnProperty("caption-text")) {
            var text = operation.insert.text;

            _this4.$emit("caption-text-removed", text);
          }
        });
      },
      ...
      checkForCustomImageGalleryHandler: function checkForCustomImageGalleryHandler() {
        this.useCustomImageGalleryHandler === true ? this.setupCustomImageGalleryHandler() : "";
      },
      checkForCustomCaptionHandler: function checkForCustomCaptionHandler() {
        this.useCustomCaptionHandler === true ? this.setupCustomCaptionHandler() : "";
      },
      ...
      setupCustomImageGalleryHandler: function setupCustomImageGalleryHandler() {
        var toolbar = this.quill.getModule("toolbar");
        toolbar.addHandler("image-gallery", this.customImageGalleryHandler); // console.log(toolbar);
      ...
      setupCustomCaptionHandler: function setupCustomCaptionHandler() {
        var toolbar = this.quill.getModule("toolbar");
        toolbar.addHandler("caption-text", this.customCaptionHandler);
        var elem = document.querySelector(".id--".concat(this.id, " .ql-caption-text"));
      ...
      customCaptionHandler: function customCaptionHandler() {
